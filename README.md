...
Each directory represents a standalone laboratory session, containing the source code (`.m` files) and implementation details.
...
| **05_Vectors** | Vector Manipulation | Indexing, Bubble Sort algorithm, statistical data analysis. |
| **06_Matrices** | Matrix Operations | 2D arrays, row/column processing, cumulative operations. |
| **07_Algorithms** | Algorithm Design | Search algorithms, Perfect numbers, Sets, Interpolation. |
| **08_Applications** | Interdisciplinary App | Linear Algebra, Signal Processing, Bioinformatics (DNA). |
| **09_Recursion** | Advanced Recursion | Recursive functions (Factorial, Fibonacci, GCD, Determinants). |
