...
| `random_binary_matrix.m` | Generates a binary matrix based on a probability threshold using nested loops, effectively simulating Bernoulli trials. |
| `findInMatrix.m` | Performs an exhaustive 2D search in a matrix to find all instances of a target value, returning an array of their $(x, y)$ coordinates. |
...
* Using nested loops (`for i... for j...`) to manipulate matrix elements and map 2D coordinates.
* Preventing runtime errors, such as division by zero, when performing statistical calculations.
* Preallocating memory for matrices and transitioning towards vectorized thinking.
